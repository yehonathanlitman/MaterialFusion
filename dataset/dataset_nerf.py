...
class DatasetNERF(Dataset):
    ...
    def _parse_frame(self, cfg, idx):
        ...
        T = torch.tensor([theta, torch.sin(phi), torch.cos(phi), r])

        return img[None, ...], mv[None, ...], mvp[None, ...], campos[None, ...], T[None, ...] # Add batch dimension
    ...
    def __getitem__(self, itr):
        ...
        if self.FLAGS.pre_load:
            img, mv, mvp, campos, T = self.preloaded_data[itr % self.n_images]
        else:
            img, mv, mvp, campos, T = self._parse_frame(self.cfg, itr % self.n_images)
