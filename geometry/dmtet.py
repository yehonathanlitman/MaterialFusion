...
class DMTetGeometry:
    def __init__(self, grid_res, scale, FLAGS, load_geometry = None):
        self.FLAGS    = FLAGS
        ...
        if load_geometry:
            base_mesh_dir = load_geometry
            print("Loading base dmtet from {}".format(base_mesh_dir))
            self.sdf    = torch.load(os.path.join(base_mesh_dir, "sdf.pth"))
            self.deform = torch.load(os.path.join(base_mesh_dir, "deform.pth"))
        else:
    ...
    def freeze_parameters(self):
        ...
        self.deform.requires_grad_(False)
